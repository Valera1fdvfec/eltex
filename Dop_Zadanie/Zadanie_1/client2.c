...
#define CLIENT2_PORT 54322  // Порт для второго клиента
...
void send_exit_message(void) {
    ...
    ip->check = 0;
    ip->check = checksum((unsigned short *)ip, sizeof(struct ip_header)/2); // Контрольная сумма IP

    // Заполняем UDP-заголовок
    udp->source = htons(CLIENT2_PORT); // Порт клиента
...
int main() {
    ...
    while (1) {
        printf("Enter message (Client 2): "); // Указываем, что это второй клиент
        fflush(stdout);
        ...
        printf("Message sent (Client 2): %s\n", input); // Подтверждение отправки
        ...
        while (1) {
            ...
            if (rupd->source != htons(SERVER_PORT) || rupd->dest != htons(CLIENT2_PORT))
                continue;
            ...
            printf("Received response (Client 2): %s\n", safe); // Вывод ответа
            break;
        }
    }

    send_exit_message(); // Отправляем "exit" перед завершением
    close(sockfd); // Закрываем сокет
    return 0;
}
